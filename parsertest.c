...
void parse(char* task_ptr) {
	while (flag == 0) {
		...
		if (strcmp(task_ptr, "vtype_token") == 0) {
			...
			task_ptr = shift_reduce(task_ptr, actionCode);
		}
		else if (strcmp(task_ptr, "num_token") == 0) {
			char* actionCode = action[peek()][num_token];
			task_ptr = shift_reduce(task_ptr, actionCode);
		}
		else if (strcmp(task_ptr, "character_token") == 0) {
			char* actionCode = action[peek()][character_token];
			task_ptr = shift_reduce(task_ptr, actionCode);
		}
		else if (strcmp(task_ptr, "boolstr_token") == 0) {
			char* actionCode = action[peek()][boolstr_token];
			task_ptr = shift_reduce(task_ptr, actionCode);
		}
		else if (strcmp(task_ptr, "literal_token") == 0) {
			char* actionCode = action[peek()][literal_token];
			task_ptr = shift_reduce(task_ptr, actionCode);
		}
			...
			char* actionCode = action[peek()][if_token];
			task_ptr = shift_reduce(task_ptr, actionCode);
		}
		else if (strcmp(task_ptr, "else_token") == 0) {
			char* actionCode = action[peek()][else_token];
			task_ptr = shift_reduce(task_ptr, actionCode);
		}
		else if (strcmp(task_ptr, "while_token") == 0) {
			char* actionCode = action[peek()][while_token];
			task_ptr = shift_reduce(task_ptr, actionCode);
		}
		else if (strcmp(task_ptr, "return_token") == 0) {
			char* actionCode = action[peek()][return_token];
			task_ptr = shift_reduce(task_ptr, actionCode);
		}
		else if (strcmp(task_ptr, "addsub_token") == 0) {
			char* actionCode = action[peek()][addsub_token];
			task_ptr = shift_reduce(task_ptr, actionCode);
		}
		else if (strcmp(task_ptr, "multdiv_token") == 0) {
			char* actionCode = action[peek()][multdiv_token];
			task_ptr = shift_reduce(task_ptr, actionCode);
		}
		else if (strcmp(task_ptr, "assign_token") == 0) {
			char* actionCode = action[peek()][assign_token];
			task_ptr = shift_reduce(task_ptr, actionCode);
		}
		else if (strcmp(task_ptr, "comp_token") == 0) {
			char* actionCode = action[peek()][comp_token];
			task_ptr = shift_reduce(task_ptr, actionCode);
		}
			...
			char* actionCode = action[peek()][comma_token];
			task_ptr = shift_reduce(task_ptr, actionCode);
		}
		else if (strcmp(task_ptr, "comma_token") == 0) {
			char* actionCode = action[peek()][comma_token];
			...
			char* actionCode = action[peek()][lparen_token];
			task_ptr = shift_reduce(task_ptr, actionCode);
		}
		else if (strcmp(task_ptr, "rparen_token") == 0) {
			char* actionCode = action[peek()][rparen_token];
			task_ptr = shift_reduce(task_ptr, actionCode);
		}
		else if (strcmp(task_ptr, "lbrace_token") == 0) {
			char* actionCode = action[peek()][lbrace_token];
			task_ptr = shift_reduce(task_ptr, actionCode);
		}
		else if (strcmp(task_ptr, "rbrace_token") == 0) {
			char* actionCode = action[peek()][rbrace_token];
			task_ptr = shift_reduce(task_ptr, actionCode);
		}
		else if (strcmp(task_ptr, "$") == 0) {
			char* actionCode = action[peek()][$];
			task_ptr = shift_reduce(task_ptr, actionCode);
			if (flag == 1) { break; }
		}
		else {
...
int main(void) {
	...
	char input[100] = "vtype id assign id semi vtype id lparen rparen lbrace return boolstr semi rbrace";
	if (strlen(input) == 0) {
